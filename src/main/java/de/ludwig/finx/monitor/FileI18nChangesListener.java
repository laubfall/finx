...
public class FileI18nChangesListener implements FileListener {
	private Logger log = Logger.getLogger(FileI18nChangesListener.class);
     ...
     * 
     * normally the hole process should be:
     * - load existing i18n-properties-files (done by app) (/)
     * - scan source files for i18n-keys (done by scanner triggerd here) (/)
     * - check if any key does not exist in the i18n-property-files (done here) (/)
     * - if there is a key that does not exist update the i18n-property-files (done here) (/)
     * - if the value of the key has changed (optional) update the i18n-propertie-files (done here)
     */
    public void fileChanged(File file) {
        ...
        for(final I18nKey k : keys) {
        	...
        	if(existingNode == null) {
        		...
        		// TODO without the meta-information I18nKeyControll it is not possible to react in a good way 
        	}
        }
    }
