 ...
 */
public class ModelBindedAccordion<T extends AccordionTitledPaneBackingBean<R>, R> extends Accordion
{
	...
	private Model<AccordionBackingBean<T>> model = new Model<>(this);

	public ModelBindedAccordion(final TitledPaneContentRenderer<R> renderer, AccordionBackingBean<T> modelObject)
	{
				...
				while (c.next()) {
					...
					if (c.wasAdded()) {
						...
						for (final T add : added) {
							...
							tp.setContent(renderer.content(add.getTitledPaneContentModelObject()));
							add.setRefComp(tp);
							tp.textProperty().bind(add.titledPaneTitleProperty());
							getPanes().add(tp);
						}
					}
				}
	 ...
	 * 
	 */
	static abstract class TitledPaneContentRenderer<B>
	{
		/**
		 * 
		 * @param modelObject
		 ...
		 */
		public abstract Node content(B modelObject);
	}
}
