...
/**
 * Highlevel Class. Stellt Zugriff auf alle I18nNodes zur Verfügung die auf
 * Basis einer Properties-Datei erzeugt wurden.
 * 
 * @author Daniel
 */
public class RootNode
{
	 ...
	 * 
	 * @param key
	 *            Immer der absolute Schlüssel! Relative Schlüssel gibt es hier
	 *            nicht! Beispiel key1: de.ludwig nun möchte man unter de.ludwig
	 *            ein Node mit Key node anlegen. Dann muss man als key
	 *            de.ludwig.node übergeben!
	 * @param iso2Language
	 * @param value
	 */
	public void addNode(final String key, final String iso2Language, final String value)
	{
		...
		if (rootNodes.isEmpty())
		{
			...
			log.debug(String.format("added node with key %s for language %s with value %s to empty root", key, iso2Language, value));
			return;
		}
		...
		for (I18nNode root : rootNodes)
		{
			...
			if (mostMatchingRootPath == null)
			{
				...
				log.debug(String.format("added node with key %s for language %s with value %s to root", key, iso2Language, value));
				continue;
			} else if (mostMatchingRootPath.key().equals(key))
			{
				mostMatchingRootPath.update(iso2Language, value);
				log.debug(String.format("updated node with key %s for language %s with value %s", key, iso2Language, value));
				return;
			} else
			{
				...
				log.debug(String.format("added node with key %s for language %s with value %s to child with key %s", key, iso2Language, value, mostMatchingKey));
				return;
			}
		}
	...
	/**
	 * Adds a node for all existing languages under the specified key. Does
	 * nothing if an {@link I18nNode} with the given key already exists.
	 * 
	...
	/**
	 * Adds a node for all existing languages under the specified key. If there are no existing languages
	 * no key is added to the structure. 
	 * 
	 ...
	 * @return {@link I18nNode} or null if no {@link I18nNode} with the given
	 *         key exists
	 */
	 ...
	 * @param key
	 * @param root
	 *            Wenn null wird direkt in den Wurzel-Nodes (rootNodes) nach
	 *            einer passenden Node geschaut. Andernfalls wird von der
	 *            übermittelten Node aus gesucht bzw. gelöscht.
	 */
